# Use BFS to search shortest path, Optimizing by saving visited. Visited is determined by state and current position
import sys
import heapq
...
def isSameState(initialState, currentState):
    for pos, blizzards in initialState.items():
        if pos not in currentState:
            return False
        b = currentState[pos]
        if len(blizzards) != len(b):
            return False
        sortedB = sorted(blizzards)
        sortedb = sorted(b)
        for i in range(len(sortedB)):
            if sortedB[i] != sortedb[i]:
                return False
    return True
